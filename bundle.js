(function (d3$1, topojson) {
  ...
  const pathGenerator = d3$1.geoPath().projection(d3$1.geoMercator());  // sets the path generator 

  // file names
  const regions = "https://lideresSocialesREST.anidiamdev.repl.co/regions",
        deaths = "https://lideresSocialesREST.anidiamdev.repl.co/deaths",
        map = "https://lideresSocialesREST.anidiamdev.repl.co/topojson_map";
  ...
  Promise.all([d3$1.csv(regions), d3$1.json(deaths), d3$1.json(map)])
      ...
      const blurElements = d => {
        ...
        d3.select(".container-descSide__departmentName")
          .text(toTitleCase(d.properties.DPTO_CNMBR + " (" + currentYear + ")"));

        d3.select(".container-descSide__totalDeaths")
          .text(() => {
            console.log(d);
          	if (deathsPY_regions[currentYear][toTitleCase(d.properties.DPTO_CNMBR)] == undefined) {
              return "There were no deaths in this department for the time you are selecting."; 
            }
      ...
      const colorMapForTotal = (d) => {
    	...
    	// generating the map
    	const dpts = topojson.feature(map, map.objects.MGN_ANM_DPTOS);  // data to topojson
    	svg.selectAll("path")
        .data(dpts.features)
    		.enter().append("path")
    			.attr("id", d => {
